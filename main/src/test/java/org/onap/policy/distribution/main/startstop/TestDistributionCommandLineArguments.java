...
public class TestDistributionCommandLineArguments {
    ...
    public void testValidateFileNameEmpty() {
        ...
        DistributionCommandLineArguments sutArgs = new DistributionCommandLineArguments(argsOnlyKeyNoValue);

        assertThatThrownBy(() -> sutArgs.validate())
            .isInstanceOf(PolicyDistributionException.class)
            .hasMessage("policy distribution configuration file was not specified as an argument");
    }

    @Test
    public void testValidateFileNameDoesNotExist() {
        String[] fileNameNotExistentArgs = {"-c", "someFileName.json"};
        DistributionCommandLineArguments sutArgs = new DistributionCommandLineArguments(fileNameNotExistentArgs);

        assertThatThrownBy(() -> sutArgs.validate())
            .isInstanceOf(PolicyDistributionException.class)
            .hasMessage("policy distribution configuration file \"someFileName.json\" does not exist");
    }

    @Test
    public void testValidateFileNameIsNotFile() {
        String[] folderAsFileNameArgs = {"-c", "src/test/resources/parameters"};
        DistributionCommandLineArguments sutArgs = new DistributionCommandLineArguments(folderAsFileNameArgs);

        assertThatThrownBy(() -> sutArgs.validate())
            .isInstanceOf(PolicyDistributionException.class)
            .hasMessage("policy distribution configuration file "
                    + "\"src/test/resources/parameters\" is not a normal file");
    }
