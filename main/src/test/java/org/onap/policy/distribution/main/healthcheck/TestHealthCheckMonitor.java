 ...
 */
public class TestHealthCheckMonitor {
    ...
    @Test
    public void testHealthCheckMonitorSuccess() throws PolicyDistributionException {
        final String reportString = "Report [name=Policy SSD, url=self, healthy=true, code=200, message=alive]";
        assertFalse(HealthCheckMonitor.healthCheckMonitor.isAlive());
        final Main main = startDistributionService();
        final HealthCheckReport report = performHealthCheck();
        validateReport(NAME, SELF, true, 200, ALIVE, reportString, report);
        assertTrue(HealthCheckMonitor.healthCheckMonitor.isAlive());
        assertTrue(HealthCheckMonitor.healthCheckMonitor.toString().startsWith("HealthCheckMonitor [servers="));
        stopDistributionService(main);
    }

    @Test
    public void testHealthCheckMonitorFailure() throws InterruptedException {
        ...
        assertTrue(HealthCheckMonitor.healthCheckMonitor.isAlive());
        assertTrue(HealthCheckMonitor.healthCheckMonitor.toString().startsWith("HealthCheckMonitor [servers="));
        HealthCheckMonitor.healthCheckMonitor.shutdown();
    }
    ...
    private HealthCheckReport performHealthCheck() {
        ...
        final HealthCheckReport response = invocationBuilder.get(HealthCheckReport.class);
        return response;
    }
