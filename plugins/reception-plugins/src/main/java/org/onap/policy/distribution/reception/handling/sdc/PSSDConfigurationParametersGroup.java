...
public class  PSSDConfigurationParametersGroup implements ParameterGroup {
    ...
    public GroupValidationResult validate() {
        ...
        if(asdcAddress == null || asdcAddress.trim().length() == 0 
            ...
            || consumerId == null || consumerId.trim().length() == 0 
            || consumerGroup == null || consumerGroup.trim().length() == 0 
            || keystorePath == null || keystorePath.trim().length() == 0
            || keystorePassword == null || keystorePassword.trim().length() == 0 ){
            validationResult.setResult("PSSDConfiguration", ValidationStatus.INVALID, 
               "the string field of PSSDConfiguration  must be a non-blank string");
        }
