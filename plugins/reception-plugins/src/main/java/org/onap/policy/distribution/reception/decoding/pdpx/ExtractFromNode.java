...
/**
 * Extract concerned info from NodeTemplate, currently ONLY HPA 
 */
public class ExtractFromNode {
    ...
    void setSdcCsarHelper(ISdcCsarHelper sdcCsarHelper) {
     ...
     */
    public PdpxPolicy extractInfo(NodeTemplate node) {
        ...
        for ( NodeTemplate nodeChild : lnodeChild) {
            LOGGER.debug("the type of this nodeChild = " + sdcCsarHelper.getTypeOfNodeTemplate(nodeChild));
        ...
        if(content.getFlavorFeatures().size() != 0){
            try(Writer writer = new FileWriter(outputFile)) {
                ...
                LOGGER.error("can't write generate policies to file " + e.getMessage());
            }
            return pdpxPolicy;
        }else {
            return null;   
 ...
 //           String id = node.getProperties().get("name");
            ...
            Attribute flavorAttribute = new Attribute("flavor_name", "");
            Directive flavorDirective = new Directive("flavor_directive");
            flavorDirective.add(flavorAttribute);
            flavorFeature.add(flavorDirective);
    ...
    private void generateBasicCapability(final CapabilityAssignment capabilityAssignment, FlavorFeature flavorFeature){
            ...
            if(virtualMemSize != null){
                ...
                flavorProperty.add(hpaFeatureAttribute);
                flavorFeature.add(flavorProperty);
            }
