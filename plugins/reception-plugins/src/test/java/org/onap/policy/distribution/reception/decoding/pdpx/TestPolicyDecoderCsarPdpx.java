...
public class TestPolicyDecoderCsarPdpx {
    ...
    public void testHpaPolicy2Vnf() throws IOException, PolicyDecodingException {
        ...
        ToscaPolicyType type = map.get(ToscaPolicyType.TOSCA_POLICY_TYPE);
        Map<String, ToscaProperty> props = type.getProperties();

        ToscaProperty onapName = (ToscaProperty)props.get(ToscaPolicyType.TOSCA_POLICY_ONAP_NAME);
        assertEquals("onapName", onapName.getDefaultValue());
        ToscaProperty policyName = (ToscaProperty)props.get(ToscaPolicyType.TOSCA_POLICY_NAME);
        assertTrue(policyName.getDefaultValue().startsWith("OOF."));
        ToscaProperty priority = (ToscaProperty)props.get(ToscaPolicyType.TOSCA_POLICY_PRIORITY);
        assertTrue(priority.getDefaultValue().contains("5"));
        ToscaProperty riskLevel = (ToscaProperty)props.get(ToscaPolicyType.TOSCA_POLICY_RISK_LEVEL);
        assertTrue(riskLevel.getDefaultValue().contains("2"));
        ToscaProperty riskType = (ToscaProperty)props.get(ToscaPolicyType.TOSCA_POLICY_RISK_TYPE);
        assertTrue(riskType.getDefaultValue().contains("Test"));
        ToscaProperty version = (ToscaProperty)props.get(ToscaPolicyType.TOSCA_POLICY_VERSION);
        assertTrue(version.getDefaultValue().contains("1.0"));

        ToscaProperty content = (ToscaProperty)props.get(ToscaPolicyType.TOSCA_POLICY_CONTENT);
        String c = (String)content.getDefaultValue();
        assertTrue(c.contains("\"policyType\":\"hpa\""));
